...
unrestricted_page_routes = {'/login', '/flag'}
...
async def status_page() -> None:
    ...
    try:
        async with httpx.AsyncClient() as client:
            r = await client.get("http://localhost:8888/flag", follow_redirects=True)
        content_label.set_text("Status for " + app.storage.user.get('referrer_path') + " : " + str(r))
...
async def flag_page() -> None:
    ui.label(f"Flag: {os.getenv('FLAG')}")
...
ui.run(storage_secret='VEERYSEEEECRET', title="Nice AI GUI", host="127.0.0.1", port=8888)
